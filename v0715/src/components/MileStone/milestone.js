...
import MilestoneInner1 from './milestoneInner1'
import MilestoneInner2 from './milestoneInner2'
                        ...
                        <div className='milestone'>
                            ...
                            <div className='infoDiv'>
                                ...
                                <div className='editDiv'>
                                    ...
                                    <button className='borderNone' onClick={() => {f.current = true; ff.current = props; setIsOpen(true)}}><span ><MdEditNote className='transformClass'/></span></button>
                                </div>
                            </div>
                            <div className='taskAddDiv'>
                                <button className='btn' onClick={() => {sessionStorage.setItem("updateMilestoneId", milestoneId); f.current = false; ff.current = null; setIsOpen(true)}}>新規タスク追加</button>
                            </div>
    ...
    const TaskChild = (props) => {
        if (dispTasks.includes(props.taskId)) {
            return (
                ...
                {(provided, snapshot) =>(
                    <div className='task' onClick={() => {sessionStorage.setItem("updateTask", JSON.stringify(props)); sessionStorage.setItem("updateMilestoneId", props.milestoneId); sessionStorage.setItem("updateTaskIndex", props.index); f.current = false; ff.current = props; setIsOpen(true)}} ref={provided.innerRef} snapshot={snapshot} {...provided.draggableProps} {...provided.dragHandleProps}>
        ...
        <>
            <div className='mainDiv'>
                ...
                <Modal isOpen={modalIsOpen} overlayClassName={{base: "overlay-base", afterOpen: "overlay-after", beforeClose: "overlay-before"}} className={{base: "content-base", afterOpen: "content-after", beforeClose: "content-before"}} onRequestClose={() => setIsOpen(false)} closeTimeoutMS={500}>
                    ...
                    {f.current ? <MilestoneInner1 {...ff.current} setIsOpen={setIsOpen}/>
                                : <MilestoneInner2 {...ff.current} setIsOpen={setIsOpen}/>}
                </Modal>
                <div className='milestoneDiv'>
                    <div className='topDiv'>
                        ...
                        <button className='btn' onClick={() => {f.current = true; ff.current = null; setIsOpen(true)}}>新規マイルストーン</button>
                    </div>
